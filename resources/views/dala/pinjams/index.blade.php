        ...
        <tbody id="pinjams_body">
            @foreach($pinjams as $pinjam)
                <tr>
                    ...
                    <a href="{{ route('dala.pinjams.edit', $pinjam->id) }}" class="btn btn-warning mx-1">Edit</a> 
                        @if($pinjam->status_dala == 'pending')
                            <!-- Approve Button -->
                            <form action="{{ route('dala.pinjams.update-status', $pinjam->id) }}" method="POST" class="d-inline">
                                @csrf
                                @method('PUT')
                                <input type="hidden" name="status_dala" value="approved">
                            ...
                            <form action="{{ route('dala.pinjams.update-status', $pinjam->id) }}" method="POST" class="d-inline">
                                @csrf
                                @method('PUT')
                                <input type="hidden" name="status_dala" value="rejected">
                                <button type="submit" class="btn btn-danger mx-1">Reject</button>
                            </form>
                        @elseif($pinjam->status_dala == 'approved' || $pinjam->status_dala == 'rejected')
                            <!-- Set to Pending Button -->
                            <form action="{{ route('dala.pinjams.update-status', $pinjam->id) }}" method="POST" class="d-inline">
                                @csrf
                                @method('PUT')
                                <input type="hidden" name="status_dala" value="pending">
                                <button type="submit" class="btn btn-secondary mx-1">Pending</button>
                            </form>
                        @endif
                    </td>
                </tr>
...
$(document).ready(function() {
    ...
    function filterPinjams(url = "{{ route('dala.pinjams.index') }}") {
                ...
                data.data.forEach(pinjam => {
                    rows += `
                        <tr>
                            ...
                            <td class="d-flex justify-content-between">
                                <a href="/dala/pinjams/${pinjam.id}/edit" class="btn btn-warning mx-1">Edit</a>
                                ${pinjam.status_dala === 'pending' ? `
                                    <form action="/dala/pinjams/${pinjam.id}/update-status" method="POST" style="display:inline-block;">
