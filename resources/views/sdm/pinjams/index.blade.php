        ...
        <tbody id="pinjams_body">
            @foreach($pinjams as $pinjam)
                <tr>
                    ...
                    <a href="{{ route('sdm.pinjams.edit', $pinjam->id) }}" class="btn btn-warning mx-1">Edit</a> 
                        @if($pinjam->status_sdm == 'pending')
                            <!-- Approve Button -->
                            <form action="{{ route('sdm.pinjams.update-status', $pinjam->id) }}" method="POST" class="d-inline">
                                @csrf
                                @method('PUT')
                                <input type="hidden" name="status_sdm" value="approved">
                            ...
                            <form action="{{ route('sdm.pinjams.update-status', $pinjam->id) }}" method="POST" class="d-inline">
                                @csrf
                                @method('PUT')
                                <input type="hidden" name="status_sdm" value="rejected">
                                <button type="submit" class="btn btn-danger mx-1">Reject</button>
                            </form>
                        @elseif($pinjam->status_sdm == 'approved' || $pinjam->status_sdm == 'rejected')
                            <!-- Set to Pending Button -->
                            <form action="{{ route('sdm.pinjams.update-status', $pinjam->id) }}" method="POST" class="d-inline">
                                @csrf
                                @method('PUT')
                                <input type="hidden" name="status_sdm" value="pending">
...
$(document).ready(function() {
    ...
    function filterPinjams(url = "{{ route('sdm.pinjams.index') }}") {
                ...
                data.data.forEach(pinjam => {
                    rows += `
                        <tr>
                            ...
                            <td class="d-flex justify-content-between">
                                <a href="/sdm/pinjams/${pinjam.id}/edit" class="btn btn-warning mx-1">Edit</a>
                                ${pinjam.status_sdm === 'pending' ? `
                                    <form action="/sdm/pinjams/${pinjam.id}/update-status" method="POST" style="display:inline-block;">
