...
@section('content')
<a href="{{ url('sdm/pinjams') }}" class="btn btn-primary mb-3">Kembali</a>
<h2>Edit Peminjaman Barang</h2>

<form action="{{ route('sdm.pinjams.update', $pinjam->id) }}" method="POST">
    @csrf
    @method('PUT')
    <div class="mb-3">
    ...
    <select name="status_sdm" id="status_sdm" class="form-select" style="color : white;" required>
    ...
    <a href="https://wa.me/{{ $peminjamWhatsappNumber }}?text=Halo {{ $noPeminjam->name }}, status SDM peminjaman dengan id : {{ $pinjam->id }} telah diubah ke {{ $pinjam->status_sdm }}." target="_blank" class="btn btn-success">Chat di WhatsApp</a>
</form>
@endsection
@section('scripts')
<script>
...
statusSelect2.addEventListener('change', updateBackgroundColor);
    ...
    function updateStockAvailability() {
        ...
        if (loanDate && returnDate) {
            $.ajax({
                url: "{{ route('sdm.pinjams.getAvailableItems') }}", // Pastikan route ini sesuai
                method: "GET",
    ...
    $('#itemModal').on('show.bs.modal', function () {
        ...
        if (loanDate && returnDate) {
            $.ajax({
                url: "{{ route('sdm.pinjams.getAvailableItems') }}",
                method: "GET",
                data: { loan_date: loanDate, return_date: returnDate },
                success: function(response) {
