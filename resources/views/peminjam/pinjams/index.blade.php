            ...
            @foreach($pinjams as $pinjam)
                <tr>
                    ...
                    <td class="d-flex justify-content-between">
                        ...
                        <form action="{{ route('peminjam.pinjams.update-status', $pinjam->id) }}" method="POST" style="display:inline-block;">
                            @csrf
                            @method('PUT')
                        ...
                        <form action="{{ route('peminjam.pinjams.update-status', $pinjam->id) }}" method="POST" style="display:inline-block;">
                            @csrf
                            @method('PUT')
                            <button type="submit" class="btn btn-success">Cancel return</button>
                        </form>
                        @else
                        <a href="{{ route('peminjam.pinjams.cek', $pinjam->id) }}" class="btn btn-warning mx-1">Edit</a> 
                        @endif
                ...
                data.data.forEach(pinjam => {
                    rows += `
                        <tr>
                            ...
                            <td class="d-flex justify-content-between">
                                ${
                                    pinjam.status === 'pending' || pinjam.status === 'rejected'
                                    ? `
                                    ...
                                    ? `
                                        <a href="/peminjam/pinjams/cek/${pinjam.id}" class="btn btn-warning mx-1">Edit</a>
                                        <form action="/peminjam/pinjams/${pinjam.id}/update-status" method="POST" style="display:inline-block;">
