...
@section('content')
<a href="{{ url('warek/pinjams') }}" class="btn btn-primary mb-3">Kembali</a>
<h2>Edit Peminjaman Barang</h2>

<form action="{{ route('warek.pinjams.update', $pinjam->id) }}" method="POST">
    @csrf
    @method('PUT')
    <div class="mb-3">
...
statusSelect2.addEventListener('change', updateBackgroundColor);
    ...
    function updateStockAvailability() {
        ...
        if (loanDate && returnDate) {
            $.ajax({
                url: "{{ route('warek.pinjams.getAvailableItems') }}", // Pastikan route ini sesuai
                method: "GET",
    ...
    $('#itemModal').on('show.bs.modal', function () {
        ...
        if (loanDate && returnDate) {
            $.ajax({
                url: "{{ route('warek.pinjams.getAvailableItems') }}",
                method: "GET",
                data: { loan_date: loanDate, return_date: returnDate },
                success: function(response) {
