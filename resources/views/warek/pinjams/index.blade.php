        ...
        <tbody id="pinjams_body">
            @foreach($pinjams as $pinjam)
                <tr>
                    ...
                    <a href="{{ route('warek.pinjams.edit', $pinjam->id) }}" class="btn btn-warning mx-1">Edit</a> 
                        @if($pinjam->status_warek == 'pending')
                            <!-- Approve Button -->
                            <form action="{{ route('warek.pinjams.update-status', $pinjam->id) }}" method="POST" class="d-inline">
                                @csrf
                                @method('PUT')
...
$(document).ready(function() {
    ...
    function filterPinjams(url = "{{ route('warek.pinjams.index') }}") {
                ...
                data.data.forEach(pinjam => {
                    rows += `
                        <tr>
                            ...
                            <td class="d-flex justify-content-between">
                                <a href="/warek/pinjams/${pinjam.id}/edit" class="btn btn-warning mx-1">Edit</a>
                                ${pinjam.status_warek === 'pending' ? `
                                    <form action="/warek/pinjams/${pinjam.id}/update-status" method="POST" style="display:inline-block;">
