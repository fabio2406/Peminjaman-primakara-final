...
<div class="container">
    ...
    <table class="table table-bordered table-hover">
        ...
        <tbody id="logs-body">
            @foreach($logs as $log)
            <tr onclick="window.location.href='{{ route('admin.logs.show', $log->id) }}'" style="cursor: pointer;">
...
$(document).ready(function() {
    ...
    function filterLogs(url = "{{ route('admin.logs.index') }}") {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                data.logs.data.forEach(log => {
                    rows += `
                        <tr onclick="window.location.href='logs/${log.id}/show'" style="cursor: pointer;">
                            <td>${log.user?.username ?? 'N/A'}</td>
                            <td>${log.pinjam_id}</td>
